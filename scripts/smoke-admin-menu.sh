...
printf '%s\n' "$admin_image_cleared_response"
printf '%s' "$admin_image_cleared_response" | node_check "result.items.some(item => item.id === '$item_id' && item.imageUrl === '')"
customer_image_cleared_response="$(json_get "$NYAN8_BASE_URL/api/customer/menu?terminal_code=$CUSTOMER_TERMINAL")"
printf '%s\n' "$customer_image_cleared_response"
printf '%s' "$customer_image_cleared_response" | node_check "result.categories.flatMap(category => category.items).some(item => item.id === '$item_id' && item.imageUrl === '')"
